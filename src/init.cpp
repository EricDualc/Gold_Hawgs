...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "auh.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "auhd.pid") + "\n";
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in AUH/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in AUH/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           auh (or specifically: darksend, instantx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in AUH/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -anonymizeauhamount=<n>     " + strprintf(_("Keep N AUH anonymized (default: %u)"), 0) + "\n";
...
/** Sanity checks
 *  Ensure that AUH is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Auhcore is shutting down."));
...
#endif
    // Make sure only a single AUH process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Auhcore is probably already running."), strDataDir));
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                boost::filesystem::path auhStateDir = GetDataDir() / "stateAuh";

                bool fStatus = boost::filesystem::exists(auhStateDir);
                const std::string dirAuh(auhStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsAuhState = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<AuhState>(new AuhState(dev::u256(0), AuhState::openDB(dirAuh, hashDB, dev::WithExisting::Trust), dirAuh, existsAuhState));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::auhMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(auhStateDir.string());
                if (fReset) {
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Auhcore") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Auhcore to complete") << "\n";
    ...
    nAnonymizeAuhAmount = GetArg("-anonymizeAuhamount", 0);
    if (nAnonymizeAuhAmount > 999999) nAnonymizeAuhAmount = 999999;
    if (nAnonymizeAuhAmount < 2) nAnonymizeAuhAmount = 2;

    LogPrintf("AUH darksend rounds %d\n", nDarksendRounds);
    LogPrintf("Anonymize Auh Amount %d\n", nAnonymizeAuhAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1Auh+1000 == (.1Auh+100)*10
       10Auh+10000 == (1Auh+1000)*10
    */
