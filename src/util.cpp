...
{
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // auh
    assert(mutexDebugLog == NULL);

    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // auh
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // auh
    if (fileout) setbuf(fileout, NULL); // unbuffered
    if (fileoutVM) setbuf(fileoutVM, NULL); // unbuffered // auh
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "auh" is a composite category enabling all AUH-related debug output
            if (ptrCategory->count(string("auh"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\AUH
// Windows >= Vista: C:\Users\Username\AppData\Roaming\AUH
// Mac: ~/Library/Application Support/AUH
// Unix: ~/.auh
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AUH";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "auh.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty auh.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override auh.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "auhd.pid"));
