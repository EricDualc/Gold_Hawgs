...
bool ToHash160(const std::string& strAuhAddress, std::string& strHash160)
{   
    CTxDestination auhAddress = DecodeDestination(strAuhAddress);
    if(IsValidDestination(auhAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&auhAddress);
...
bool ToAuhAddress(const std::string& strHash160, std::string& strAuhAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strAuhAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToAuhAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToAuhAddress(tokenEvent.receiver, tokenEvent.receiver);
