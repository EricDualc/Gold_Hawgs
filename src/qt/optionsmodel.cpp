...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::AUH);
    ...
    if (!settings.contains("nAnonymizeAuhAmount"))
        settings.setValue("nAnonymizeAuhAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeAuhAmount = settings.value("nAnonymizeAuhAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAuhAmount"))
        SoftSetArg("-anonymizeauhamount", settings.value("nAnonymizeAuhAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in auh.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAuhAmount:
            return QVariant(nAnonymizeAuhAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAuhAmount:
            nAnonymizeAuhAmount = value.toInt();
            settings.setValue("nAnonymizeAuhAmount", nAnonymizeAuhAmount);
            emit anonymizeAuhAmountChanged(nAnonymizeAuhAmount);
            break;
        case CoinControlFeatures:
