...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasAuhAddress = IsValidDestination(addr);
        if (hasAuhAddress) {
            ui->label->setText("Address (Auh)");
        ...
        isAddressValid = hasHexAddress || hasAuhAddress;
    }
