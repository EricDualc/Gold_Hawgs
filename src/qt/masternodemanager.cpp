...
static void NotifyAuhNodeUpdated(MasternodeManager *page, CAuhNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateAuhNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyAuhNodeChanged.connect(boost::bind(&NotifyAuhNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyAuhNodeChanged.disconnect(boost::bind(&NotifyAuhNodeUpdated, this, _1));
}
...
void MasternodeManager::updateAuhNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        for (PAIRTYPE(std::string, CAuhNodeConfig) adrenaline : pwalletMain->mapMyAuhNodes)
        {
            updateAuhNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditAuhNode* aenode = new AddEditAuhNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CAuhNodeConfig c = pwalletMain->mapMyAuhNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    AuhNodeConfigDialog* d = new AuhNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyAuhNodes.erase(sAddress);
        walletdb.EraseAuhNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    for (PAIRTYPE(std::string, CAuhNodeConfig) adrenaline : pwalletMain->mapMyAuhNodes)
    {
        CAuhNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
