...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::AUH)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            if (!fEnableAuhsend) {
                ui->toggleObfuscation->setText(tr("Start Auhsend"));
            } else {
                ui->toggleObfuscation->setText(tr("Stop Auhsend"));
            }
...
void SmartContractPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("AUH")
    updateDisplayUnit();
}
...
void SmartContractPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeAuhAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAuhAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeAuhAmount = strAnonymizeAuhAmount.remove(strAnonymizeAuhAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAuhAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeAuhAmount * COIN) nMaxToAnonymize = nAnonymizeAuhAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeAuhAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeAuhAmount));
        strAnonymizeAuhAmount = strAnonymizeAuhAmount.remove(strAnonymizeAuhAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAuhAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
