...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setAuhSchedule(auhDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = auhDGP.getBlockSize(nHeight);
    minGasPrice = auhDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = auhDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    AuhTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractAuhTX resultConverter;
    if(!convert.extractionAuhTransactions(resultConverter)){
    ...
    for(AuhTransaction auhTransaction : auhTransactions){
        txGas += auhTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + auhTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(auhTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, auhTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    // Found a solution (stake)
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AUHMiner : generated block is stale");

        for(const CTxIn& vin : pblock->vtx[1].vin) {
            if (wallet.IsSpent(vin.prevout.hash, vin.prevout.n)) {
                return error("AUHMiner : Gen block stake is invalid - UTXO spent");
            }
        }
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("AUHMiner : ProcessNewBlock, block not accepted");
    }
