...
AuhState::AuhState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = AuhState::openDB(_path + "/auhDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

AuhState::AuhState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute AuhState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, AuhTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createAuhAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            auh::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> AuhState::vins() const // temp
{
...
void AuhState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* AuhState::vin(dev::Address const& _a) const
{
    return const_cast<AuhState*>(this)->vin(_a);
}

Vin* AuhState::vin(dev::Address const& _addr)
{
...
// void AuhState::commit(CommitBehaviour _commitBehaviour)
// {
...
void AuhState::kill(dev::Address _addr)
{
...
void AuhState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address AuhState::createAuhAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void AuhState::deleteAccounts(std::set<dev::Address>& addrs){
...
void AuhState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void AuhState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
