...
bool AuhDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule AuhDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t AuhDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AuhDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AuhDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AuhDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AuhDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AuhDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AuhDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AuhDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void AuhDGP::createParamsInstance(){
...
dev::Address AuhDGP::getAddressForBlock(unsigned int blockHeight){
...
void AuhDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AuhDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AuhDGP::parseStorageOneUint64(uint64_t& value){
...
void AuhDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AuhDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
