    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Auh and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Auh per transaction on average.
            */
            if (r <= 20) {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready) {
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeAuhAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if (!sessionFoundMasternode) {
        ...
        double fAuhSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Auh CENT - sessionTotalValue %d\n", fAuhSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom) {
    ...
    //
    // bit 0 - 100Auh+1 ( bit on if present )
    // bit 1 - 10Auh+1
    // bit 2 - 1Auh+1
    // bit 3 - .1Auh+1
    // bit 3 - non-denom
        ...
        if (c % 60 * (nLiquidityProvider + 1) == 0) {
            if (nLiquidityProvider != 0) {
                ...
                if (nRand == 50 + nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8) {
                    ...
                    nAnonymizeAuhAmount = (rand() % nLeftToAnon) + 3;
                } else {
