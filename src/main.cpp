...
// Copyright (c) 2015-2017 The AUH developers
...
#if defined(NDEBUG)
#error "AUH cannot be compiled without assertions."
#endif
...
std::unique_ptr<AuhState> globalState = nullptr;
...
std::string SCVersion ("/Auhcore:1.0.0/");
...
unsigned int dgpMaxBlockSize = 2000000; // auh
...
const string strMessageMagic = "Auh Signed Message:\n";
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            AuhDGP auhDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = auhDGP.getMinGasPrice(chainActive.Height() + 1);
            uint64_t blockGasLimit = auhDGP.getBlockGasLimit(chainActive.Height() + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            AuhTxConverter converter(tx, NULL);
            ExtractAuhTX resultConverter;
            if(!converter.extractionAuhTransactions(resultConverter)){
            ...
            std::vector<AuhTransaction> auhTransactions = resultConverter.first;
            std::vector<EthTransactionParams> auhETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(AuhTransaction auhTransaction : auhTransactions){
                sumGas += auhTransaction.gas() * auhTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, auhTransaction.gasPrice());
                } else {
                    txMinGasPrice = auhTransaction.gasPrice();
                }
                VersionVM v = auhTransaction.getVersion();
                if(v.format!=0)
                ...
                if(auhTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(auhTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(auhTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)auhTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(auhETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > auhTransactions.size())
...
//#if 0
      if (pindex->nHeight > Params().FirstSCBlock()) {
        globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // auh
        globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // auh
...
#if 0
    AuhDGP auhDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setAuhSchedule(auhDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//auhDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//auhDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//auhDGP.getBlockSize(pindex->nHeight + 1);
...
///////////////////////////////////////////////////////////////////////////////////////// auh
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                AuhTxConverter convert(tx, &view, &block.vtx);

                ExtractAuhTX resultConvertAuhTX;
                if(!convert.extractionAuhTransactions(resultConvertAuhTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertAuhTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertAuhTX.first, blockGasLimit);
                ...
                for(AuhTransaction& ltx : resultConvertAuhTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertAuhTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertAuhTX.first[k].from(), resultConvertAuhTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
            if (globalState == nullptr)
                return error("%s: globalState is not yet initialized!\n", __func__);
            oldHashStateRoot = dev::h256(globalState->rootHash()); // auh
            oldHashUTXORoot = dev::h256(globalState->rootHashUTXO()); // auh
        }
        ...
        if (!rv) {
            ...
            if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
                globalState->setRoot(oldHashStateRoot); // auh
                globalState->setRootUTXO(oldHashUTXORoot); // auh
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 AUH
 ...
 * As an example, Auhcore uses /Auhcore:X.X.X/ version string. So, this function will
 * return 5 for /Auhcore:5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion Auhcore wallet version as a string
 * @return Major Auhcore major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    AuhTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<AuhTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path auhDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(auhDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(auhDir.string(), std::ios::in | std::ios::out);
...
bool AuhTxConverter::extractionAuhTransactions(ExtractAuhTX& auhtx){
    std::vector<AuhTransaction> resultTX;
    ...
    auhtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool AuhTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool AuhTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
AuhTransaction AuhTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    AuhTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = AuhTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = AuhTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
