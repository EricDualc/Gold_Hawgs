...
*Setup instructions for a gitian build of AUH using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AUH
...
Clone the git repositories for auh and gitian and then checkout the auh version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the auh repository
...
To build AUH (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the auh repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/auh/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/auhcore/auh
...
For example:
```bash
URL=https://github.com/crowning-/auh.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit auh=${COMMIT} --url auh=${URL} ../auh/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit auh=${COMMIT} --url auh=${URL} ../auh/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit auh=${COMMIT} --url auh=${URL} ../auh/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/auh-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/auh-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/auh-build.assert
```
...
[auh/gitian.sigs](https://github.com/auhcore/gitian.sigs/) repository, or if that's not possible to create a pull
request.
